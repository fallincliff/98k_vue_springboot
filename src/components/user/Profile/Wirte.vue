...
export default {
  ...
  methods: {
    ...
    insertText(tid,author,path,tag,picture){
      const _this = this;
      this.$axios.get('http://localhost:8888/text/insert?tid='+tid+'&author='+author+'&path='+path+'.html&tag='+tag+'&picture='+picture).then((res) => {
    ...
    //添加博文txt
    addSomeText(name,lines){
      //console.log(name+lines)
      const _this = this;
      this.$axios.get('http://localhost:8888/file/uploadText?name='+name+ "&textarea2="+lines).then((res) => {
        console.log(res.data);
      }).catch(
        function (error) {
          // 请求失败处理
    ...
    addSomePeoplePicsForm: function (index) {
      ...
      formData.append('file', this.FL[index]) // 'file' 这个名字要和后台获取文件的名字一样;
      ...
      this.$axios({
        ...
        _this.insertText(tid,temp.name,_this.input1,_this.dynamicTags,_this.picturename);
    ...
    addsomeHtml(filename,file){
      ...
      //console.log(document.querySelector('input[type=file]').files[0])
      this.$axios({
        url: 'http://localhost:8888/file/uploadFile',   //****: 你的ip地址
        data: file,
        method: 'post',
        headers: {
          'Content-Type': 'multipart/form-data',
